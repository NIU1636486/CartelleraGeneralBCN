/**
 * Verdi Barcelona Service
 * Handles fetching and parsing Verdi cinema data
 */

import fetch from 'node-fetch';
import { VerdiParser } from '../parsers/verdi-parser.js';
import { DataStore } from '../utils/data-store.js';
import { TMDBService } from './tmdb-service.js';

export class VerdiService {
    constructor() {
        this.parser = new VerdiParser();
        this.dataStore = new DataStore('verdi');
    ...
    /**
     * Fetch and parse Verdi data for a given month
    ...
    async getMoviesForMonth(month, forceRefresh = false) {
        try {
            ...
            if (!forceRefresh) {
                ...
                if (cachedData) {
                    console.log(`Using cached Verdi data for ${month}`);
                    return cachedData.movies;
                }
            }

            console.log(`Fetching fresh data from Verdi Barcelona...`);
            ...
            console.log(`Enriching ${groupedMovies.length} Verdi movies with metadata...`);
            const enrichedMovies = await this.tmdbService.enrichMovies(groupedMovies);

            // Save to storage
            this.dataStore.saveMonthData(month, enrichedMovies, 'Verdi Barcelona');

            console.log(`Parsed and enriched ${enrichedMovies.length} Verdi movies for ${month}`);
            return enrichedMovies;
        } catch (error) {
            console.error(`Error fetching Verdi data for ${month}:`, error);

            // Try to return cached data even if expired
            const cachedData = this.dataStore.getMonthData(month);
            if (cachedData) {
                console.log('Returning stale cached Verdi data due to error');
                return cachedData.movies;
            }
